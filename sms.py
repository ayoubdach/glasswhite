...
import subprocess, sqlite3, datetime, secrets, hashlib, time
import re
...
# Database setup - FIXED columns
def init_db():
    conn = sqlite3.connect('sms_fixed_full.db')
    c = conn.cursor()
    ...
    # Users table - FIXED: removed is_admin column that was causing error
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .error { background: #f8d7da; color: #721c24; padding: 10px; border-radius: 10px; margin: 10px 0; }
        .info { background: #d4edda; color: #155724; padding: 10px; border-radius: 10px; margin: 10px 0; }
...
<body>
    <div class="card">
        ...
        {% if not has_password %}
        <div class="info">🔑 First time! Create your password below.</div>
        {% endif %}
...
<body>
    ...
    <script>
        ...
        async function loadHistory() {
            ...
            let data = await res.json();
            let html = '';
            data.history.forEach(h => { html += `<tr><td>${h.timestamp}</td><td>${h.number}</td><td>${h.message.substring(0,50)}</td><td>${h.status}</td>`; });
            document.getElementById('historyTable').innerHTML = html;
        }
        
        function useNumber(number) { document.getElementById('number').value = number; bootstrap.Modal.getInstance(document.getElementById('contactsModal')).hide(); }
        function selectQuickContact() { let select = document.getElementById('quickContact'); document.getElementById('number').value = select.value; }
        function showAddContact() { new bootstrap.Modal(document.getElementById('contactModal')).show(); }
        function showContacts() { let contacts = document.querySelectorAll('#contactsList .list-group-item'); let html = '<div class="list-group">'; contacts.forEach(c => { html += c.outerHTML; }); html += '</div>'; document.getElementById('contactsModalList').innerHTML = html; new bootstrap.Modal(document.getElementById('contactsModal')).show(); }
        ...
        loadStats(); loadContacts(); loadHistory();
    </script>
</body>
</html>
'''
...
if __name__ == '__main__':
    print("="*60)
    print("🔐 SMS GATEWAY - FULL VERSION (FIXED)")
    print("="*60)
    print(f"📱 Local URL: http://localhost:8080")
    print("="*60)
    print("👥 USERS:")
    print("   • admin, user1, user2, user3, user4, user5")
    print("   • First login: Create your own password")
    print("   • Password is private (only you know it)")
    print("="*60)
